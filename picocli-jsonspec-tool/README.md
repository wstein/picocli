# picocli-jsonspec-tool

A picocli command line tool for working with [`picocli-jsonspec`](../picocli-jsonspec) DSL/JSON
...
Unlike the specs it operates on, this outer tool is not itself described by a picocli-jsonspec
file — a spec-authoring tool has no reason to avoid the annotation processor it's built with.

## Usage

```
$ java -jar picocli-jsonspec-tool.jar <subcommand> ...
```
...
```
$ java -jar picocli-jsonspec-tool.jar preview ../picocli-jsonspec/examples/flix-0.60.0.picocli
...
`picocli-jsonspec` itself has exactly one dependency, `picocli` core (`api rootProject`), matching
...
- There's no `convert` subcommand yet (DSL ↔ JSON): `CommandSpecJson.write()` already covers
  ...
  *writer* in `picocli-jsonspec` (currently parse-only) — not yet implemented.
- No execution wiring, same as `picocli-jsonspec` itself: this tool only renders a spec, it never
  runs the CLI it describes.

See `SpecLoaderTest`, `PreviewTest`, `CompletionTest`, `ManpageTest`, `ValidateTest`, and
`JsonSpecToolAppTest` (the last one exercises every subcommand end-to-end through
