...
From version 4.8, picocli can also generate a completion script for the fish shell.
...
(See <<Installing Completion Scripts Permanently in Bash/ZSH/Fish>>.)
...
[source,fish]
----
$ mycommand generate-completion --shell=fish | source
----
...
The `--shell` option defaults to `bash` and also accepts `fish`.
...
== Installing Completion Scripts Permanently in Bash/ZSH/Fish
...
=== Fish
Fish looks for completion scripts in `~/.config/fish/completions/`, so the simplest way to install a fish completion script permanently is to save it there under the name of the command it completes:

[source,fish]
----
$ mycommand generate-completion --shell=fish > ~/.config/fish/completions/mycommand.fish
----

Fish loads scripts from this directory automatically for every new shell; there is no need to separately `source` the file, and no `bashcompinit`-style shim is needed (fish's `complete` builtin is native, not a bash emulation).
...
TIP: Pass `--shell=fish` to the `picocli.AutoComplete` build-time tool to generate a fish script instead of bash/zsh.
When `--completionScript`/`-o` is omitted, the generated file is named `<commandName>.fish` rather than `<commandName>_completion`.
