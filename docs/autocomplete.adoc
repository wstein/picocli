...
NOTE: The `picocli.AutoComplete` build-time tool shown below only generates bash/zsh scripts.
To generate a fish script during the build, call `picocli.AutoComplete.fish(String, CommandLine)`
from your own build step, or generate it at runtime with `generate-completion --shell=fish`
as described above.
